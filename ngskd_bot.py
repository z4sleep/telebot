...
def get_text_messages(message):
    ...
    if message.text.lower() in ["привет","hi","hi","hello","здарова","шалом"]:
        ...
        bot.send_message(message.chat.id, "Поздаровайся или спроси меня то что я знаю! Например время, дату или ...")
        ...
        bot.send_message(message.chat.id, f"На вопрос - {message.text} я отвечу {datetime.now().time()}")
    elif message.text.lower() in ["какое число","какое сегодня число","дата","назови дату","скажи дату","date","current data"]:
        bot.send_message(message.chat.id, f"На вопрос - {message.text} я отвечу {datetime.now().date()}")
    else:
        bot.send_message(message.chat.id, f"Я не понимаю что значит - {message.text} \nНапиши /help")
